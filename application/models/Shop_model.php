...
class Shop_model extends CI_Model {
    ...
    */
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    $Records[] = $Record;
                    if (in_array('products', $Params)) {
                        $Records[$key]['products'] = $this->Products_model->get_products('product_name,product_description,min_price,max_price,product_main_photo',array('product_ids' => explode(",",$Record['product_ids'])),TRUE);                    
                        unset($Records[$key]['product_ids']);
                    }
                ...
                $Record = $Query->row_array();
                if (in_array('products', $Params)) {
                    $Record['products'] = $this->Products_model->get_products('product_name,product_description,min_price,max_price,product_main_photo',array('product_ids' => explode(",",$Record['product_ids'])),TRUE);                    
                    unset($Record['product_ids']);
                }
                if (in_array('product_gallery_images', $Params)) {
    ...
    function set_shop($client_id, $Input = array()) {
        ...
        //echo"<pre>";print_r($client_shop_products_array);exit;
